package com.sdt.tv.launcher.features;
...
import com.sdt.tv.launcher.util.ClearUtil;
        ...
        startFound.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View arg0) {
                ...
                if (!found) {
                    ...
                    new FoundTask(Environment.getExternalStorageDirectory() + "/", CLEAR_TYPE).execute();
                } else {
                    if (fileGrbagesize != 0) {
                        startClear.setClickable(false);
                        ...
                        startFound.setClickable(true);
                        Toast.makeText(GarbageClear.this, "当前不需要清理", Toast.LENGTH_SHORT).show();
                    }
                }
            ...
            for (File file : files) {
                if (file.isFile()) {
                    ...
                    for (int i = 0; i < CLEAR_TYPE.length; i++) {
                        if (file.getPath().substring(file.getPath().length() - extension[i].length())
                                .equals(extension[i])) {
                            filePath = file.getAbsolutePath();
                            isInstall = ClearUtil.TakeIsInstallApk(filePath, GarbageClear.this);
                            // 判断是否已安装װ
                            if (!isInstall) {
